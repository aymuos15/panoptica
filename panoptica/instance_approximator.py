...
class InstanceApproximator(SupportsConfig, metaclass=ABCMeta):
    ...
    def approximate_instances(
        ...
        **kwargs,
        ...
        pred_labels = pred_labels if len(pred_labels) > 0 else (0, 0)
        ref_labels = ref_labels if len(ref_labels) > 0 else (0, 0)
        min_value = min(np.min(pred_labels), np.min(ref_labels))
        assert (
            min_value >= 0
