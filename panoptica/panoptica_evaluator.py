...
def panoptic_evaluate(
    ...
    **kwargs,
    ...
    if isinstance(processing_pair, EvaluateInstancePair):
        ...
        if hasattr(processing_pair, "num_pred_instances") and instance_metadata["original_num_preds"] == 0:
            instance_metadata["original_num_preds"] = processing_pair.num_pred_instances
        if hasattr(processing_pair, "num_ref_instances") and instance_metadata["original_num_refs"] == 0:
        ...
        has_many_to_one_mappings = processing_pair.num_pred_instances < instance_metadata["original_num_preds"]

        # Use effective counts if many-to-one mappings were detected, otherwise use original counts
        final_num_pred_instances = (
            processing_pair.num_pred_instances if has_many_to_one_mappings else instance_metadata["original_num_preds"]
        )
        final_num_ref_instances = processing_pair.num_ref_instances if has_many_to_one_mappings else instance_metadata["original_num_refs"]
