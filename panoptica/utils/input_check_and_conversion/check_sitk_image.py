...
class SITKImageChecker(_InputDataTypeChecker):
    ...
    def sanity_check_images(self, prediction_image: sitk.Image, reference_image: sitk.Image, *args, **kwargs) -> tuple[bool, str]:
        ...
        if (np.array(prediction_image.GetOrigin()) - np.array(reference_image.GetOrigin())).sum() > self.threshold:
            return False, "Origin Mismatch: {} vs {}".format(prediction_image.GetOrigin(), reference_image.GetOrigin())

        if (np.array(prediction_image.GetSpacing()) - np.array(reference_image.GetSpacing())).sum() > self.threshold:
            return False, "Spacing Mismatch: {} vs {}".format(prediction_image.GetSpacing(), reference_image.GetSpacing())

        if (np.array(prediction_image.GetDirection()) - np.array(reference_image.GetDirection())).sum() > self.threshold:
